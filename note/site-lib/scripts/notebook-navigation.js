...
function bindSiteNavigation(){const site=window.ObsidianSite;if(!site||typeof site.onDocumentLoad!=='function'||site.__mathcraftNotebookNavigationBound)return false;site.__mathcraftNotebookNavigationBound=true;let currentPath=document.querySelector('meta[name="pathname"]')?.getAttribute('content')??'';site.onDocumentLoad(documentView=>{const nextPath=documentView?.pathname??'';if(!nextPath||normalizePath(nextPath)===normalizePath(currentPath))return;currentPath=nextPath;syncNotebook(nextPath);});return true;}
